...
export class ValveComponent implements OnInit {
  ...
  private getDataPointsTemperatureSet(deviceLogs: DeviceLog[]): ValveDataPoint[] {
    let dataPoints = deviceLogs.reduce(
      (acc: ValveDataPoint[], log, index) => {
        // 1. Filter log if difference with next log is less than granularity to avoid overlapping
        const nextLog = deviceLogs[index + 1];
        if (nextLog?.date) {
          const diff = (nextLog.date.getTime() - log.date.getTime()) / (60 * 1000);
          if (diff < chartGranularity) {
            return acc;
          }
        }

        // 2. Map log to chart data format
        const dataPoint: ValveDataPoint = {
          x: log.date,
    ...
    // 3. Don't add first log at fromDate because temperature set value is not known before first log

    // 4. Add last log at currentDate to have a continuous line
    const lastDataPoint = dataPoints.at(-1);
    if (lastDataPoint) {
      dataPoints = [
        ...dataPoints,
